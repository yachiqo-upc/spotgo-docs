...
- **Electronic Receipt:** Documento fiscal digital asociado a una operación elegible de un Driver y generado con la información tributaria disponible.
- **Electronic Invoice (Comprobante electrónico):** Documento fiscal digital generado para una operación que requiere información tributaria empresarial, como un RUC, y que conserva su relación con el Digital Payment correspondiente.
- **Nearby Parking Zones (Parking Zones cercanas):** Parking Zones registradas en SpotGo que la aplicación muestra alrededor de la ubicación del Driver. SpotGo proporciona las coordenadas, disponibilidad, horarios, precios y demás detalles operativos; Google Maps API proporciona el mapa base integrado y sus funciones de representación, sobre el cual SpotGo muestra las ubicaciones de sus Parking Zones.
...
- **Firebase Cloud Messaging:** Servicio externo utilizado para entregar notificaciones push a las aplicaciones móviles. El backend decide qué evento debe comunicarse y Firebase Cloud Messaging entrega el mensaje al dispositivo registrado; no reemplaza el registro de notificaciones de SpotGo.
...
- **Driver (Conductor):** Usuario con una cuenta propia creada mediante la aplicación móvil, con credenciales de SpotGo o Google Authentication. Administra su User Profile y sus Vehicles, consulta Availability, crea Reservations y consulta Nearby Parking Zones.
...
| Description | **As a** Developer,<br>**I want** RESTful services to expose Parking Zones, User Profiles, Vehicles, Occupancy Status, Reservations, Parking Sessions, Digital Payments and Billing data,<br>**so that** the mobile application can synchronize the information required by each actor. |
...
| Description | **As a** Developer,<br>**I want** the billing service to generate and update the applicable Electronic Invoice for approved Digital Payments, Additional Charges and Refunds,<br>**so that** SpotGo can provide a consistent fiscal record for each registered Driver operation. |
| Acceptance Criteria | **Scenario 1: Generate Electronic Receipt**<br>**Given** an approved Digital Payment exists for a registered Driver and the Driver provides a valid DNI<br>**When** the billing service processes the operation<br>**Then** it generates and associates an *Electronic Receipt* with the payment and the corresponding Reservation or linked Parking Session of the registered Driver.<br><br>**Scenario 2: Generate Electronic Invoice**<br>**Given** the requester requires an *Electronic Invoice* and provides a valid RUC<br>**When** the billing service processes the approved Digital Payment<br>**Then** it generates an *Electronic Invoice* and associates it with the provided tax information.<br><br>**Scenario 3: Additional Charge or Refund**<br>**Given** a Reservation or linked Parking Session of a registered Driver generates an approved Additional Charge or an authorized Refund<br>**When** the billing service processes the adjustment<br>**Then** it generates or updates the associated Electronic Receipt, retains the relationship with the original operation, and reflects the adjusted amount. |
