...
- **Driver (Conductor):** Usuario con una cuenta propia creada mediante el registro público de la aplicación móvil, con credenciales propias o una cuenta externa autorizada. Administra su User Profile y sus Vehicles, consulta Availability y crea Reservations.
...
- **Guest Parking Session (Sesión de estacionamiento de invitado):** Registro creado manualmente por un Staff cuando un Guest ingresa al estacionamiento. Conserva la placa ingresada manualmente, el Parking Spot asignado, las horas de ingreso y salida, el monto calculado y la confirmación operativa del pago físico.
- **High Capacity (Alta capacidad):** Estado operativo que se activa cuando la ocupación total supera el 95 % de los Parking Spots operativos disponibles y requiere una alerta para el Staff.
...
- **Receipt (Comprobante):** Documento o confirmación asociado con una operación de SpotGo. Puede ser un Virtual Receipt, Electronic Receipt o Electronic Invoice, según la finalidad y la información tributaria disponible.
...
- **Reservation Status (Estado de reserva):** Estado comercial y operativo de una Reservation. Puede ser *Pending Payment*, *Reserved*, *Active*, *Completed*, *Cancelled*, *No-show*, *Overstayed* o *Payment Rejected*; *Pending Payment* y *Payment Rejected* representan la intención y el resultado del pago.
...
- **Staff (Personal operativo):** Usuario administrativo cuya cuenta y rol STAFF son provisionados internamente por un SuperAdmin y asociados con un Tenant. Cuenta con vistas administrativas y funciones operativas; es distinto del Driver y no se crea mediante el registro público.
...
- **SuperAdmin (Administrador de plataforma):** Usuario encargado de registrar Tenants, crear las cuentas iniciales de los Staff y administrar la configuración de los clientes B2B.
...
- **Unauthorized Parking (Estacionamiento indebido):** Ocupación no validada que el Staff confirma como uso no autorizado después de revisar la operación y las condiciones del estacionamiento.
- **Unauthorized Parking Alert (Alerta por estacionamiento indebido):** Alerta operativa generada cuando un Parking Spot presenta Unvalidated Occupancy. Requiere revisión y clasificación por parte del Staff.
...
- **User Profile (Perfil de usuario):** Perfil operativo asociado con una identidad. En un Driver determina las Parking Zones que puede utilizar; en un Staff representa sus capacidades administrativas y su asignación a un Tenant. No es un perfil que se transfiera de un Driver a un Staff.
- **Vehicle (Vehículo):** Medio de transporte asociado con un Driver y utilizado para ocupar un Parking Spot. Un Driver puede registrar varios Vehicles y seleccionarlos en sus Reservations; cada Vehicle conserva la placa registrada y sus datos descriptivos.
...
- **Google Authentication:** Mecanismo externo que permite a un Driver crear una cuenta o iniciar sesión mediante una cuenta de Google. No crea cuentas de Staff ni de SuperAdmin.
